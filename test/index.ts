...
describe('ProtoBufJs', () => {
  it('works', () => {
    ...
    var jsonProto: ProtoBuf.ProtoBuilder = ProtoBuf.loadJson(readFileSync(`${__dirname}/test.json`,
      { "encoding": "utf8" }));
    assertIsProtoBuilder(jsonProto, "loadJson");

    var jsonFileProto: ProtoBuf.ProtoBuilder = ProtoBuf.loadJsonFile(`${__dirname}/test.json`);
    assertIsProtoBuilder(jsonFileProto, "loadJsonFile");

    ProtoBuf.loadJsonFile(`${__dirname}/test.json`, (error: any, builder: ProtoBuf.ProtoBuilder) => {
      assertIsProtoBuilder(builder, "loadJsonFile callback");
    });

    var proto: ProtoBuf.ProtoBuilder = ProtoBuf.loadProto(readFileSync(`${__dirname}/test.proto`,
      { "encoding": "utf8" }));
    assertIsProtoBuilder(proto, "loadProto");

    var protoFile: ProtoBuf.ProtoBuilder = ProtoBuf.loadProtoFile(`${__dirname}/test.proto`);
    assertIsProtoBuilder(protoFile, "loadProtoFile");

    ProtoBuf.loadProtoFile(`${__dirname}/test.proto`, (error: any, builder: ProtoBuf.ProtoBuilder) => {
...
function assertIsDotProto(dp: ProtoBuf.DotProto, name: string) {
  ...
  assertIsParser(new dp.Parser(readFileSync(`${__dirname}/test.proto`, { "encoding": "utf8" })),
    name + ".Parser");
  assertIsTokenizer(new dp.Tokenizer(readFileSync(`${__dirname}/test.proto`, { "encoding": "utf8" })),
    name + ".Tokenizer");
}
...
function assertIsProtoEnum(pe: ProtoBuf.ProtoEnum, name: string) {
  ...
  pe.values.forEach(value => assertIsProtoEnumValue(value, name + ".values"));
}
